    ...
    public static class BigIntegerExtensions
    {
        ...
        public static BigInteger ToBigInteger(this string str)
        {
            if (str.Contains("e", StringComparison.OrdinalIgnoreCase))
            {
                ...
                int exponent = int.Parse(pieces[1]);
                if (pieces[0].Length - 2 <= exponent)
                {
                    ...
                    if (exponent > 1000)
                    {
                        string[] decimalPieces = pieces[0].Split('.');
                        string baseNumber = decimalPieces[0];
                        ...
                        BigInteger bigInt = BigInteger.Parse(baseNumber, NumberStyles.Float);
